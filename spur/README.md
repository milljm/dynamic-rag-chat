...
- **Projects** — Coding toggle (assistant). Named fences persist in `vector_dir/projects/workspace/`, or in a directory added via **Add project dir**. The model writes local workers under `agents/` and `<RUN:file.py args>` executes Python or Node (no shell).
