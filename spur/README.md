...
- **Projects** — one row per project root. **Tools** live outside the project and persist. **Add project dir** registers a directory in place. If it is not git, Coding asks before `git init`. `<GIT:status>` is local-only. `<TOOL:uv_setup.py>` runs a toolkit script with cwd in the project.
