...
Each **branch** owns collections named `{branch}_user_documents` and `{branch}_ai_documents`. Story mode can also read an un-prefixed **gold** collection (imports and chat attachments; not cloned on fork). Assistant gold is `assistant_gold_documents`. Text attachments are stored there on the turn they arrive so later turns can retrieve them.
