    ...
    *Usage:*
    ...
    def store_data(self, data,
        ...
        *Returns:*
        ...
        try:
            retriever.add_documents([doc], ids=[parent_id])
    ...
    def delete_entries(self, collection: str, ids: list[str]) -> int:
        ...
        *Returns:*
        ...
        try:
            ...
            child_ids = []
            for cid, meta in zip(payload.get('ids') or [], payload.get('metadatas') or []):
                if cid in wanted_set:
                    child_ids.append(cid)
                    continue
                if isinstance(meta, dict) and meta.get('doc_id') in wanted_set:
                    child_ids.append(cid)
            if child_ids:
                # pylint: disable=protected-access
                vector._collection.delete(ids=child_ids)
                # pylint: enable=protected-access
                removed += len(child_ids)
        except Exception:  # pylint: disable=broad-exception-caught
            pass
        try:
            self._docstore(collection).mdelete(wanted)
        except Exception:  # pylint: disable=broad-exception-caught
            pass
        return removed
